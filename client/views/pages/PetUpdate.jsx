...
export default function PetUpdate() {
  ...
  function careRequestUpdate() {
    ...
    fetch("/careRequestUpdate.json", {
      ...
      console.log('careRequestUpdate() /careRequestUpdate.json result : ', data);
      alert('수정이 완료 되었습니다.');
      navigate('/pages/petDetail', { state: { care_request_number: location.state.care_request_number } });
    }).catch((error) => {
      console.log('careRequestUpdate() /careRequestUpdate.json error : ', error);
    });
  }
  ...
  return (
    <FormStyled>
      <div>
        <label>제목</label>
        <input type="text" value={careRequest.care_request_title} onChange={(e) => careRequestChange(e, 'care_request_title')} />
      </div>
      <div>
        <label>내용</label>
        <input type="text" value={careRequest.care_request_content} onChange={(e) => careRequestChange(e, 'care_request_content')} />
      </div>
      <div>
        <label>이름</label>
        <input type="text" value={careRequest.pet_name} onChange={(e) => careRequestChange(e, 'pet_name')} />
      </div>
      <div>
        <label>나이</label>
        <input type="text" value={careRequest.pet_age} onChange={(e) => careRequestChange(e, 'pet_age')} />
      </div>
      <div>
        <label>성별</label>
        <select value={careRequest.pet_sex} onChange={(e) => careRequestChange(e, 'pet_sex')}>
      ...
      <div>
        <label>동물종류</label>
        <input type="text" value={careRequest.pet_animal_type} onChange={(e) => careRequestChange(e, 'pet_animal_type')} />
      </div>
      <div>
        <label>품종</label>
        <input type="text" value={careRequest.pet_animal_kind} onChange={(e) => careRequestChange(e, 'pet_animal_kind')} />
      </div>


      <Button color={"#dddd"} title={"수정"} onClick={careRequestUpdate} />
    </FormStyled>
  );
}
