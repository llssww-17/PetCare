...
//reqest 정보를 받을 시, Body의 값을 받아올 수 있도록 해주는 express 설정
webServer.use(express.json());
webServer.use(express.urlencoded({ extended: false }));

webServer.listen(8030, function () {
  console.log('8030Port Nodejs Express를 활용한 Web Server 구동');
});
...
webServer.post('/logout.json', function (request, response) {
  ...
  try {
    ...
    console.log('session destroy error23 : ', err);
    response.json({isSuccess: false, message: '로그아웃 처리 에러'});
  }
});
...
// ROOT URL, Router's, 화면요청 URL 등.. 이 외 나머지 정적 자원에 대한 처리 기능
...
// //비동기처리 : 오래걸리는 작업은 나중으로 미뤄놓고 백그라운드에서 처리 후, 결과를 응답해줌
// //비동기 처리 결과 받는 방법
// //1. Callback 함수 활용
// //2. Promise 객체 활용
